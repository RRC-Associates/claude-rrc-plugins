...
You enabled auto-update during setup (Step 4), so Claude Code will check for new versions at the start of each session and prompt you to run `/reload-plugins` when one is available. That's the normal path.

**To force a refresh manually** — for example if you know a change just landed and you don't want to wait for the next session — run these two commands at the prompt:
...
The first command pulls the latest catalog and plugin versions from the GitHub repo; the second loads the new versions into your active session.

::: callout-note
**If `/reload-plugins` doesn't seem to pick up the new version**, fully quit the Claude Code app and restart it. On Windows, "X-ing out" of the window isn't always enough — open Task Manager and end the `Claude` process (or any background helper processes), then relaunch. The CLI version usually picks up changes after `/reload-plugins` alone; the GUI app sometimes needs a full restart.
:::
