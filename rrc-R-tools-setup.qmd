...
Close and reopen Claude Code (CLI or App GUI), then start a new session. Ask Claude: "What skills do you have available?" You should see `r-style` and `diagnose-before-fixing` in the response. `quarto-render-sync` and `quarto-scaffold` are on-demand skills — they won't appear in the list but are available when you need them (just describe what you want or use `/quarto-scaffold` or `/quarto-render-sync` directly).

## Keeping the Plugin Current

By default, Claude Code only auto-checks for updates from Anthropic's own marketplaces — third-party marketplaces like `claude-rrc-plugins` have auto-update disabled out of the box. To opt in (one-time setup):
...
With auto-update enabled, Claude Code checks for new versions at session startup. If an update is found, you'll be prompted to run `/reload-plugins` to activate it in the current session.

**To force a refresh manually**, run these two commands at the prompt:
